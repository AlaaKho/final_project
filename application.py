...
def overdue():
    ...
    delivery_dates = db.execute(
        "SELECT book_id, user_id, delivery_date FROM requests WHERE delivery_date=:today AND request_type='borrow' ", today=str(today))

    # iterate all of the requests that should be inserted into borrowed table today.
    for i in range(len(delivery_dates)):
        copies = db.execute("SELECT * FROM borrowed WHERE user_id=:user_id AND operation=1 AND book_id=:book_id AND transacted=:today", user_id=delivery_dates[i]["user_id"], book_id=delivery_dates[i]["book_id"],
                            today=delivery_dates[i]['delivery_date'])
        ...
        if len(copies) == 0:
            ...
            db.execute("UPDATE books SET state=:state WHERE id=:id",
                       state="on loan", id=delivery_dates[i]["book_id"])

            send_email(delivery_dates[i]["user_id"], delivery_dates[i]
                       ["book_id"], operation="borrowed")
    ...
    for i in range(len(stl_borrowed)):
        expiry_date = datetime.datetime.strptime(
            stl_borrowed[i]["transacted"], "%Y-%m-%d") + datetime.timedelta(days=(stl_borrowed[i]["lendfor"]))

        if datetime.datetime.strptime(str(today), "%Y-%m-%d") >= expiry_date and stl_borrowed[i]["state"] != "overdue":
            db.execute("UPDATE books SET state=:state WHERE id=:id",
                       state="overdue", id=stl_borrowed[i]["book_id"])
...
def send_email(user_id, book_id, operation):
    ...
    t2 = threading.Thread(target=send_onthread, args=[
        app, msg, user_email, book_details, operation])
    t2.setDaemon(True)
    t2.start()
...
def index():
    ...
    if request.method == "POST":
        ...
        brows = db.execute(
            "SELECT * FROM books WHERE title=:title AND subtitle=:subtitle", title=title, subtitle=subtitle)
        ...
        if len(brows) >= 1:
            ...
            for i in range(len(brows)):
                ...
                book_id = brows[i]["id"]
                rows = db.execute("SELECT * FROM mycollection WHERE book_id=:book_id AND user_id=:user_id",
      ...
      # update the user mycollection table with the book ID
        db.execute("INSERT INTO mycollection (user_id, book_id) VALUES (:user_id, :book_id)",
...
def delete():
    ...
    db.execute("DELETE FROM mycollection WHERE book_id=:book_id",
...
def edit():
    ...
    if request.form.get("lendfor_edit") != "":
        ...
        db.execute("UPDATE books SET lendfor=:lendfor WHERE id=:id",
                   lendfor=nloan_period, id=book_id)

    # update notes
    if request.form.get("booknotes_edit") != "":
        nbooknotes = request.form.get("booknotes_edit")
        db.execute("UPDATE books SET notes=:notes WHERE id=:id",
                   notes=nbooknotes, id=book_id)

    # update review
    if request.form.get("review_edit") != "":
        nreview = request.form.get("review_edit")
        db.execute("UPDATE books SET review=:review WHERE id=:id",
                   review=nreview, id=book_id)
...
def Return():
    ...
    rows = db.execute(
        "SELECT * FROM requests WHERE book_id=:book_id AND request_type='Return' AND state='pending'", book_id=book_id)
    if len(rows) != 0:
    ...
    # else
    owner_id = db.execute(
        "SELECT user_id FROM mycollection WHERE book_id=:book_id", book_id=book_id)
    owner_id = owner_id[0]["user_id"]
    return_date = datetime.timedelta(
        days=int(request.form.get("return_date"))) + datetime.date.today()
...
def requests():
    ...
    if request.method == "POST":
        ...
        request_info = db.execute(
            "SELECT book_id, user_id, request_type FROM requests WHERE id=:id", id=request_id)
        ...
        if request.form.get("approve"):
            ...
            delivery_date = datetime.timedelta(
                days=int(request.form.get("dispatch_date"))) + approval_time
        ...
        if request.form.get("confirm"):
            ...
            db.execute("INSERT INTO borrowed (user_id, operation, book_id, transacted) VALUES (:user_id, :operation, :book_id, :transacted)",
                       user_id=requester_id, operation=-1, book_id=book_id, transacted=approval_time)
...
def register():
    ...
    if request.method == "POST":
        ...
        # if new registrant into our website
            ...
            session["user_id"] = db.execute("INSERT INTO users (username, email, password) VALUES (:username, :email, :password)",
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        trgt_email = db.execute("SELECT * FROM users WHERE email=:email",
                                email=request.form.get("email"))
...
def change_password(token=None):
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET password=:password WHERE email=:email",
                   password=hashed_password, email=session["email"])
